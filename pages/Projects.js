...
function Projects({ pinnedItems }) {
  const [projects] = useState(pinnedItems);
            ...
            >
              ...
              <div className="relative mx-5">
                ...
                <div className="absolute top-auto flex items-center justify-center pb-2 inset-1">
                  {item.object && (
                    <p className="flex px-3 py-1 m-1 text-xs font-bold text-gray-800 bg-teal-500 border rounded-full shadow-lg cursor-pointer border-cyan-600 hover:text-blue-900 md:text-sm">
                      <span className="pr-1">Commits: </span>
                      {item.object.history.totalCount}
                    </p>
                  )}
                  {item.cloneCount && (
                    <p className="inline-block px-3 py-1 m-1 text-xs font-bold text-gray-800 bg-teal-500 border rounded-full shadow-lg cursor-pointer border-cyan-600 hover:text-blue-900 md:text-sm">
              ...
              <div className="flex items-center justify-center pb-3">
                <Link href={item.url} passHref target="_blank" rel="noopener noreferrer">
                  <span className="inline-block px-3 py-1 m-1 text-xl font-semibold rounded-full text-textSecondary bg-textTertiary hover:bg-tertiary">
                    <AiFillGithub />
                  </span>
                </Link>
                {item.homepageUrl ? (
                  <Link href={item.homepageUrl} passHref target="_blank" rel="noopener noreferrer">
                    <span className="inline-block px-3 py-1 m-1 text-xl font-semibold rounded-full text-textSecondary bg-textTertiary hover:bg-tertiary">
                      <GrDeploy />
                    </span>
                  </Link>
